...
fromPaCo2Tree (PaCo2.Bin x l r) = Bin x' (fromPaCo2Tree l) (fromPaCo2Tree r)
  where PaCo2.Node { PaCo2.skip = k, PaCo2.string = v, PaCo2.label = p } = x
        bs = map (\n -> v `testBit` (31 - n)) [0 .. pred k]
        x' = Node { list = bs
                  , pref = p
                  }
fromPaCo2Tree PaCo2.Tip         = Tip
